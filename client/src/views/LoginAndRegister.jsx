...
const LoginAndRegister = () =>{
    ...
    const loginUser = async (values) =>{
        try{
            const login = await axios.post('http://localhost:8000/api/auth/login', values);
            ...
            Swal.fire({
                title: 'Usuario o Contraseña inválida, por favor revise e intente de nuevo',
                icon: 'error',
                confirmButtonText: 'Ok',
        })
    }
    ...
    return(
        <div>
            ...
            />
            <Button variant="success" className="my-2" onClick={handleLogin}>{!isLogin ? 'Acceder' : 'Registrarse'}</Button>
                    </Col>
                </Row>
            </Container>
        </div>
