...
module.exports.getBookByUser = async (req, res) =>{
    ...
    return res.status(500).json({ error: err,
        msg: 'error al traerse los libros por el User'
        });
    }
};
...
module.exports.getOneBookById = async (req, res) => {
    try {
    const { id } = req.params;
    const book = await (await Books.findById(id)).populate('author', 'firstName lastName role email');
...
module.exports.updateBookById = async (req, res) => {
    ...
    const { id } = req.params;
    const { body } = req;
    const updateTheBook = await Books.findOneAndUpdate({ _id: id }, body, {
        new: true,
    });
    return res.json({
